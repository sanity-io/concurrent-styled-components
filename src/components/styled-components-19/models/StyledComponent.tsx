...
import {forwardRef, type Ref, use, useDebugValue, useInsertionEffect} from 'react'
...
const shouldUseInsertionEffect = !IS_BROWSER ? false : true
...
function useInjectedStyle<T extends ExecutionContext>(
  ...
  return [className, insertionEffectBuffer] as [string, [name: string, rules: string[]][]]
}
...
function useStyledComponentImpl<Props extends object>(
  ...
  const [generatedClassName, cssBuffer] = useInjectedStyle(componentStyle, context)
  console.log({generatedClassName, cssBuffer, componentStyle, context, forwardedComponent, props})
  ...
  console.log({ElementToBeCreated, propsForElement})

  return (
    <>
      {Array.isArray(cssBuffer) &&
        cssBuffer.length > 0 &&
        cssBuffer.map(([name, rules]) => (
          <style key={name} precedence={`sc-scoped`} href={`sc-${name}`}>
            {rules.join('/n')}
          </style>
        ))}
...
>(
   ...
   */
  const WrappedStyledComponent = forwardRef(forwardRefRender) as unknown as IStyledComponent<
    'web',
    any
  > &
    Statics
